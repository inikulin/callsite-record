...
CallsiteRecord.fromError = function (error, topFrameFilter) {
    var stackFrames = stackParser.parse(error);

    if (typeof topFrameFilter === 'function') {
        while (stackFrames.length) {
            if (!topFrameFilter(stackFrames[0]))
                stackFrames.shift();
            else
                break;
        }
...
// API
module.exports = function createCallsiteRecord (/* err, topFrameFilter || fnName, typeName */) {
